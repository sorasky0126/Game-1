    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-4 flex items-center justify-center">
      <div className="max-w-2xl w-full bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-2xl">
        <div className="text-center mb-6">
          <h1 className="text-4xl font-bold text-white mb-2">⛓️ チェインブレイカー</h1>
          <p className="text-blue-200 text-sm">同じ色を繋げて消そう！3個以上で得点</p>
        </div>

        <div className="flex justify-between items-center mb-6 bg-white/20 rounded-2xl p-4">
          <div className="text-center">
            <div className="text-yellow-300 text-sm font-semibold">レベル</div>
            <div className="text-white text-2xl font-bold">{level}</div>
          </div>
          <div className="text-center">
            <div className="text-green-300 text-sm font-semibold">スコア</div>
            <div className="text-white text-2xl font-bold">{score}</div>
          </div>
          <div className="text-center">
            <div className="text-red-300 text-sm font-semibold">残り手数</div>
          ...
          <div className="inline-block bg-white/20 rounded-xl p-2">
            {selected.length > 0 && (
              <span className="text-white font-semibold">
                選択中: {selected.length}個 → {selected.length >= 3 ? `${selected.length * selected.length * 10}点` : '3個以上必要'}
              </span>
            )}
            {selected.length === 0 && (
              <span className="text-blue-200">同じ色の隣接セルをタップして繋げよう</span>
            )}
          </div>
        </div>
        ...
        {gameOver && (
          <div className="mt-6 bg-red-500/90 text-white rounded-2xl p-6 text-center">
            ...
            <p className="mb-4">レベル{level} - 最終スコア: {score}点</p>
            <p className="text-sm mb-4">目標スコア {500 * level}点に届きませんでした</p>
            <button
              onClick={resetGame}
        ...
        <div className="mt-6 bg-white/10 rounded-2xl p-4">
          <h3 className="text-white font-bold mb-2">📖 ルール</h3>
          <ul className="text-blue-200 text-sm space-y-1">
            <li>• 同じ色の隣接するセルを繋げて選択</li>
            <li>• 3個以上繋げると消せる（得点 = 個数² × 10）</li>
            <li>• 15手以内に目標スコアを達成するとレベルアップ</li>
            <li>• レベルが上がると使える色が増えて難しくなる</li>
            <li>• 長いチェインを作ると高得点！</li>
          </ul>
        </div>
      </div>
